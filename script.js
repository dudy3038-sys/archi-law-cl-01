...
// 1) parkResultBox에 자동으로 문구를 쓰는 placeholder 로직 제거
// 2) 주차 법정대수 산정 입력값을 "건축개요 UI"에서 가져올 수 있게 전역 getter 제공
// ✅ FIX: 용도 2개 이상일 때 usageAreas가 1개만 가는 문제 해결
//   - boot() 순서: bindUses() -> bindUsageAreaTable()
//   - usageAreaTable render에서 sel.value를 r.use에도 즉시 반영
...
(() => {
  ...
  function bindUsageAreaTable() {
    ...
    function render() {
      ...
      _usageRows.forEach(r => {
        ...
        // ✅ FIX: 화면에 반영된 선택값을 r.use에도 즉시 동기화
        sel.value = r.use || (options[0]?.label || "");
        r.use = sel.value; // <- 핵심 수정
    ...
    function addRow() {
      _usageRows.push({
        id: cryptoRandomId(),
        use: _uses[0]?.label || "", // ✅ 이제 _uses가 먼저 생성되므로 정상 세팅됨
        area: "0",
        note: "",
      });
      render();
  ...
  function bindBuildingOverviewInputs() {
    ...
    const p = $("parkingArea");
    if (p) p.addEventListener("blur", () => { p.value = format2(p.value); });
  ...
  function boot() {
    ...
    // ✅ FIX: _uses를 먼저 만든 뒤, usage area table을 바인딩해야 첫 행 use가 빈값으로 안 남음
    bindUses();
    bindUsageAreaTable();
